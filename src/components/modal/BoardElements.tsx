    ...
    return (
        <>
            <div className="w-80 h-[72vh] rounded-xl bg-white relative z-10 overflow-y-scroll py-2 pl-2 pr-2" onClick={(e) => {
                ...
                {/* elements */}
                {selectedCard && selectedCard.boardElement.reverse().map((item: IBoardElement) => {
                    return (
                        <div key={item.id} className={`boardElement w-full h-fit mb-2 rounded-md border-slate-300 border`}
                            onClick={() => {
                                dispatch(selectElementId(item.id));
                            }}
                        >
                            {/* buttons */}
                            {selectedElementId === item.id && <section className="boardElement w-full h-10 flex gap-2 items-center justify-center">
                                <Button text="刪" handleClick={() => {
                                ...
                                <Button text="中" handleClick={() => {

                                }} classProps="bg-slate-300" />
                                <Button text="鎖" handleClick={() => {

                                }} classProps="bg-slate-300" />
                                <Button text="隱" handleClick={() => {

                                }} classProps="bg-slate-300" />
                                <Button text="下" handleClick={() => {

                                }} classProps="bg-slate-300" />
                                <Button text="上" handleClick={() => {
